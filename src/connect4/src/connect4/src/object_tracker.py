...
##### Edit These Lines  ########################################################
    ...
    COLOR_MIN = (73,81,0) # HSV for baxter hand
...
################################################################################
  ...
  def img_received(self, message):
	...
	if largest_poly is not None:
	  ...
	  self.check_piece_color(7, 6, 2.5, 1.9, 3.5, 3.2)
	  print "Hue vector"
  ...
  def check_piece_color(self,nx,ny,offx,offy,lengthx,lengthy):
	...
	#Right now only sampling one pixel, need to take mean of pixels around center
        color_swatch = self.hsv_img[pix_center[1]-3:pix_center[1]+3,pix_center[0]-3:pix_center[0]+3,0]
        print "swatch"
        print color_swatch
        color_val = np.mean(color_swatch)
        self.piece_mean.append(color_val)
        if color_val < 5: #red
          self.piece_color.append("red")
        elif color_val > 24 and color_val < 31: #yellow
          self.piece_color.append("yellow")
        else: #nothing
          self.piece_color.append("none")
